...
class Inky_Pdf extends FPDF{
	...
	*******************************************************************************/
	function Image($file,$x,$y,$w=0,$h=0,$type='',$link='', $isMask=false, $maskImg=0){
		...
		if ($isMask) $x = $fwPt;// + 10; // embed hidden, ouside the canvas  
...
function ImagePngWithAlpha($file,$x,$y,$w=0,$h=0,$link='')
{
	...
	while ($xpx<$wpx){
		...
		while ($ypx<$hpx){
			...
			imagesetpixel($alpha_img, $xpx, $ypx, (int)$alpha);
	    ++$ypx;
		}
		++$xpx;
	}
...
function RotatedEpsColorable($file , $x , $y , $w , $h , $angle , $imageColor)
	{
		//Image rotated around its upper-left corner
		$this->Rotate($angle,$x,$y);
		$this->ImageEpsColorable($file, $x, $y, $imageColor, $w, $h, $link='', true);
		$this->Rotate(0);
	}
...
/* Modify style and select corresponding font*/
 ...
 function CellFit($w,$h=0,$txt='',$border=0,$ln=0,$align='',$fill=0,$link='',$scale=0,$force=1)
	...
	function ImageEpsColorable ($file, $x, $y, $imageColor, $w=0, $h=0, $link='', $useBoundingBox=true){	
		$data = file_get_contents($file);
		if ($data===false) $this->Error('EPS file not found: '.$file);
...
if($colorable == 'true')
{
$this->ImageEpsColorable($file, $x, $y-$posY, $imageColor, $w, $h, $link='', true);
}
else
{
...
if($colorable == 'true')
{
$this->ImageEpsColorable($file, $x-$posX, $y, $imageColor, $w, $h, $link='', true);
}
else
{
...
if($colorable == 'true')
{
$this->ImageEpsColorable($file, $x-$posX, $y-$posY, $imageColor, $w, $h, $link='', true);
}
else
{
