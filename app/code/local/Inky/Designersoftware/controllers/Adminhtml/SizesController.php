...
class Inky_Designersoftware_Adminhtml_SizesController extends Mage_Adminhtml_Controller_action
{
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				...
				//To update all designs having this one size
				$this->updatePriceInAllDesigns($model);
	...
	public function updatePriceInAllDesigns($model){
		$partName = $model->getTitle();
		$partPrice = $model->getPrice();
		//echo "<pre>";print_r($model->getData());exit;
		Mage::getSingleton('adminhtml/session')->setPriceUpdateDesignCode(false);
		//echo "<pre>";print_r($model->getData());exit;
		$collection = Mage::getModel('designersoftware/designersoftware')->getCollection()
							->addFieldToSelect('designersoftware_id')
							->addFieldToSelect('price_info_arr')
							->addFieldToSelect('design_data_array')
							//->addFieldToFilter('gallery_status',1)
							->addFieldToFilter('price_info_arr',array('like' => '%' . $model->getTitle() . '%'));
									
		//echo $collection->getSelect();exit;
		if($collection->getSize() > 0){
			foreach($collection as $data){
				$priceInfoArr = $data->getPriceInfoArr();
				$designDataArray = $data->getDesignDataArray();
				$unSerializePriceInfoArr = unserialize($priceInfoArr);
				$unSerializeDesignDataArray = unserialize($designDataArray);
				//echo "<pre>";print_r($unSerializeDesignDataArray);exit;
				for($i=0;$i<count($unSerializePriceInfoArr);$i++){
					if($unSerializePriceInfoArr[$i]['partIdendifire']=='option' && $unSerializePriceInfoArr[$i]['partName']==$partName){
						$unSerializePriceInfoArr[$i]['partPrice']=$partPrice;
						break;
					}
				}
				for($i=0;$i < count($unSerializeDesignDataArray);$i++){
					for($j=0;$j < count($unSerializeDesignDataArray[$i]);$j++){
						//echo "<pre>";print_r($unSerializeDesignDataArray[$i][$j]);
						if($unSerializeDesignDataArray[$i][$j]['shapeType']=='Option' && $unSerializeDesignDataArray[$i][$j]['objTitle']==$partName){
							$unSerializeDesignDataArray[$i][$j]['price']=$partPrice;
							break;
						}
					}			
				}
				//echo "<pre>";print_r($unSerializeDesignDataArray);exit;
				$serializePriceInfoArr=serialize($unSerializePriceInfoArr);
				$serializeDesignDataArray=serialize($unSerializeDesignDataArray);

				$data->setPriceInfoArr($serializePriceInfoArr)
							->setDesignDataArray($serializeDesignDataArray)
							->save();
			}
		}
		return;
